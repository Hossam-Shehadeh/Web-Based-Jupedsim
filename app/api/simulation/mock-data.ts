...
import { isInsideWalkableArea, isPointNearObstacle } from "@/lib/geometry"

// Helper function to generate mock simulation frames
export function generateMockSimulationFrames(elements: Element[], simulationTime: number, timeStep: number) {
  const frames = []
  ...
  for (const agent of agents) {
    ...
    const path = generatePath(agent.position, target, elements, numFrames)
...
function generatePath(start: Point, end: Point, elements: Element[], steps: number): Point[] {
  const path: Point[] = []
